...
2. `projects/{nn}.{project-name}/design-system/visual-direction.mdc` — 비주얼 방향 (Section 6 기준)
   (파일이 없으면 Section 6을 생략하고 그 사실을 보고서에 명시한다)
3. `projects/{nn}.{project-name}/context/screen-specs/{screen-id}-*.md` — 데이터 필드, 액션, 상태 기준
4. `projects/{nn}.{project-name}/prompt/{screen-id}-*-prompt.md` — 의도한 실행 계획
...
### 6. 비주얼 품질 (visual-direction.mdc 기준)
> `visual-direction.mdc`를 읽고 체크 항목을 동적으로 도출한다.
> 이 섹션의 항목은 고정값이 아니다. 각 프로젝트의 visual-direction.mdc 내용에서 파생한다.
>
> **도출 방법**: visual-direction.mdc의 각 섹션(비주얼 톤, Elevation & Surface, 강조 방식, 밀도, 확정 디자인 결정 등)을
> 순서대로 읽고, 이 화면에서 위반 가능한 항목을 체크리스트로 만들어 검토한다.
> 항목별로 위반 여부와 출처 섹션을 함께 명시한다.
...
| 체크 항목 | 기준 출처 | 결과 |
|----------|----------|------|
| (visual-direction.mdc에서 도출) | visual-direction.mdc #{섹션} | ✅ / ❌ |
